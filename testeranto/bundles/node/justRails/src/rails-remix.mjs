...
var implementation = {
  ...
  whens: {
    ATodoIsAdded: (title) => {
      return async ([domain, x, railsPort2, remixPort2], pm) => {
        await fetch(`http://${domain}:${railsPort2}/todos`, {
          method: "POST",
          headers: {
  ...
  thens: {
    ...
    RailsHas404Page: async () => {
      return async (domain, railsPort2, remixPort2, pm) => {
        const backend = `http://${domain}:${railsPort2}/ThisIsNotAPath`;
    ...
    ThereIsARemixFrontPage: async () => {
      return async (domain, railsPort2, remixPort2, pm) => {
        const frontend = `http://${domain}:${remixPort2}`;
        const p = await pm.newPage();
    ...
    TheGreetingsAre: async (expected) => {
      return async (domain, railsPort2, remixPort2, pm) => {
        const remixFrontPage = `http://${domain}:${remixPort2}`;
        const p = await pm.newPage();
...
var tInterface = {
  ...
  beforeEach: async (domain, givenInititializer, tr, iv, pm) => {
    ...
    await poll(railsHealth, (n) => n === 200);
    const remixHealth = `http://localhost:${tr2.ports[1]}`;
    await poll(remixHealth, (n) => n === 200);
  ...
  butThen: async ([domain, railsSidecar, railsPort2, remixPort2], thenCb, tr, pm) => {
    await (await thenCb())(domain, railsPort2, remixPort2, pm);
    return [domain, railsSidecar, railsPort2, remixPort2];
  }
};
...
var rails_remix_default = Node_default(
  "localhost",
  spec,
  implementation,
  tInterface
...
export {
  rails_remix_default as default,
  implementation,
  tInterface
};
